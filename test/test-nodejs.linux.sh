...
assert_clean_env() {
    ...
    assert "! -e /tmp/node-v14.15.4-linux-x64.tar.xz" \
 	   "Installation file exists: /tmp/node-v14.15.4-linux-x64.tar.xz"
    assert "! -e /tmp/node-v18.12.1-linux-x64.tar.xz" \
 	   "Installation file exists: /tmp/node-v18.12.1-linux-x64.tar.xz"
}
...
should_successfully_use_default_version_if_no_version_given() {
    ...
    assert "${PATH// /§} = $HOME/opt/node-v14.15.4-linux-x64/bin:${SPT_ORIGINAL_PATH// /§}" \
	   "PATH is not correct: $PATH"
    assert "-e /tmp/node-v14.15.4-linux-x64.tar.xz" \
	   "Installation file should exist"
    assert "-e $HOME/opt/node-v14.15.4-linux-x64" \
 	   "Installation dir should exist: $HOME/opt/node-v14.15.4-linux-x64"
	
    reset_path_vars
    rm /tmp/node-v14.15.4-linux-x64.tar.xz
    rm -rf $HOME/opt/node-v14.15.4-linux-x64
}
...
should_successfully_use_given_version() {
    ...
    assert "${SETUP_NODEJS_ORIGINAL_PATH// /§} = ${SPT_ORIGINAL_PATH// /§}" \
	   "SETUP_NODEJS_ORIGINAL_PATH != SPT_ORIGINAL_PATH: $SETUP_NODEJS_ORIGINAL_PATH"
    assert "${PATH// /§} = $HOME/opt/node-v18.12.1-linux-x64/bin:${SPT_ORIGINAL_PATH// /§}" \
	   "PATH is not correct: $PATH"
    assert "-e /tmp/node-v18.12.1-linux-x64.tar.xz" \
	   "Installation file should exist"
    assert "-e $HOME/opt/node-v18.12.1-linux-x64" \
 	   "Installation dir should exist: $HOME/opt/node-v18.12.1-linux-x64"
    
    reset_path_vars
    rm /tmp/node-v18.12.1-linux-x64.tar.xz
}
...
should_skip_installation_and_set_paths_if_already_installed() {
    ...
    reset_path_vars
    rm -rf $HOME/opt/node-v18.12.1-linux-x64
}
...
should_abort_installation_if_installation_fails() {
    assert_clean_env
    touch /tmp/node-v14.15.4-linux-x64.tar.xz
