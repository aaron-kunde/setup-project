...
assert_no_tmp_global_vars_set() {
    ...
    assert "-z $DEFAULT_JDK_VERSION" \
	   "DEFAULT_JDK_VERSION is set: $DEFAULT_JDK_VERSION"
    assert "-z $JDK_VERSION" \
	   "JDK_VERSION is set: $JDK_VERSION"
}

assert_clean_env() {
    assert_no_tmp_global_vars_set
    assert "-z $SETUP_JDK_ORIGINAL_PATH" \
	   "SETUP_JDK_ORIGINAL_PATH is set: $SETUP_JDK_ORIGINAL_PATH"
...
should_successfully_use_default_version_if_no_version_given() {
    ...
    assert_no_tmp_global_vars_set
    assert "${SETUP_JDK_ORIGINAL_PATH// /§} = ${SPT_ORIGINAL_PATH// /§}" \
	"SETUP_JDK_ORIGINAL_PATH != SPT_ORIGINAL_PATH: $SETUP_JDK_ORIGINAL_PATH"
