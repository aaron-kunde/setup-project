...
should_successfully_use_default_version_if_no_version_given() {
    ...
    assert "$OPTIND -eq 1" \
	   "OPTIND != 1: $OPTIND" 
    assert "$INSTALLATION_BASE_DIR = $HOME/opt" \
	   "INSTALLATION_BASE_DIR != HOME/opt: $INSTALLATION_BASE_DIR"
    assert "$DEFAULT_TMPL_VERSION = default_version" \
	   "DEFAULT_TMPL_VERSION != default_version: $DEFAULT_TMPL_VERSION"
    assert "$TMPL_VERSION = default_version" \
	   "TMPL_VERSION != default_version: $TMPL_VERSION"
    assert "${SETUP_TMPL_ORIGINAL_PATH// /§} = ${SPT_ORIGINAL_PATH// /§}" \
	   "SETUP_TMPL_ORIGINAL_PATH != SPT_ORIGINAL_PATH: $SETUP_TMPL_ORIGINAL_PATH"
    assert "${PATH// /§} = ${INSTALLATION_BASE_DIR// /§}/tmpl-default_version:${SPT_ORIGINAL_PATH// /§}" \
...
should_successfully_use_given_version() {
    ...
    assert "$DEFAULT_TMPL_VERSION = default_version" \
	   "DEFAULT_TMPL_VERSION != default_version: $DEFAULT_TMPL_VERSION"
    assert "$TMPL_VERSION = 0" \
	   "TMPL_VERSION != 0: $TMPL_VERSION"
    assert "${SETUP_TMPL_ORIGINAL_PATH// /§} = ${SPT_ORIGINAL_PATH// /§}" \
	   "SETUP_TMPL_ORIGINAL_PATH != SPT_ORIGINAL_PATH: $SETUP_TMPL_ORIGINAL_PATH"
    assert "${PATH// /§} = ${INSTALLATION_BASE_DIR// /§}/tmpl-0:${SPT_ORIGINAL_PATH// /§}" \
	   "PATH is not correct: $PATH"
    assert "-e $INSTALLATION_FILE" \
	   "Installation file should exist"
...
should_skip_installation_and_set_paths_if_already_installed() {
    ...
    assert "$DEFAULT_TMPL_VERSION = default_version" \
	   "DEFAULT_TMPL_VERSION != default_version: $DEFAULT_TMPL_VERSION"
    assert "$TMPL_VERSION = installed" \
	   "TMPL_VERSION != installed: $TMPL_VERSION"
    assert "${SETUP_TMPL_ORIGINAL_PATH// /§} = ${SPT_ORIGINAL_PATH// /§}" \
	   "SETUP_TMPL_ORIGINAL_PATH != SPT_ORIGINAL_PATH: $SETUP_TMPL_ORIGINAL_PATH"
    assert "${PATH// /§} = ${INSTALLATION_BASE_DIR// /§}/tmpl-installed:${SPT_ORIGINAL_PATH// /§}" \
	   "PATH is not correct: $PATH"
    assert "! -e $INSTALLATION_FILE" \
